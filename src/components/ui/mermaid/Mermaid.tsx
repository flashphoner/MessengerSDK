...
const Mermaid: FC<MermaidProps> = memo(({ code, onRender }) => {
  ...
  return (
    <div style={{minHeight: 450, position: "relative"}}
      ...
      {svg ? (
        <div
          className="mermaid-svg transition-opacity duration-200"
          style={{opacity: svg ? 1 : 0}}
          dangerouslySetInnerHTML={{__html: svg}}
        />
      ) : (
        <div className="bg-gray-200 animate-pulse w-full h-[140px]"/>
      )}
    </div>
  );
});
